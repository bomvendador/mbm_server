...
def save_orders_for_protocol(request):
    if request.method == 'POST':
        ...
        for order in json.loads(protocol_ok_arr[0]):
            ...
            protocol_orders_db.decision = order['decision']
...
def save_orders_for_protocol_file(request):
    ...
    #     response = {}
        ...
        for order in orders[0]:
            ...
            appointed_for_ok = AppointedForOK.objects.filter(ready_for_OK=ready_for_ok_db).latest('added')
            ...
            if order['Решение'] == 'Перенос':
                ...
                ready_for_ok_db.marked_for_next_ok = True
                ready_for_ok_db.save()
                order_db = Order.objects.get(number=order['Номер заявки'])
                order_db.status = OrderStatus.objects.get(name='Готово для ОК')
